...
struct virtq_avail {
  ...
  uint16_t ring[]; // descriptor numbers of chain heads
};
...
struct virtq_used {
  uint16_t flags; // always zero
  volatile uint16_t idx;   // device increments when it adds a ring[] entry
...
struct virtio_net_config { 
  ...
  uint16_t status;                // only exists if VIRTIO_NET_F_STATUS is set
  uint16_t max_virtqueue_pairs;   // only exists if VIRTIO_NET_F_MQ is set
  uint16_t mtu;                   // only exists if VIRTIO_NET_F_MTU is set
};
